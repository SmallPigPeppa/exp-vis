...
# 绘图
fig, axs = plt.subplots(1, 3, figsize=(24 * 0.95, 6))
...
legend_fontsize = 30
marker_size = 11  # Adjust marker size
...
def plot_data(ax, x, y_vanilla, y_flexivit, y_mspe, bit_depth, yticks):
    ...
    ax.set_title(f'$height = {bit_depth * 16}$', fontsize=title_fontsize)
    ax.invert_xaxis()
    ax.legend(fontsize=legend_fontsize)
    ax.tick_params(axis='both', labelsize=tick_label_fontsize)
    ax.set_xlabel(r'$width$', fontsize=axis_label_fontsize)
...
plot_data(axs[0], x2, y2_vanilla, y2_flexivit, y2_mspe, 12, yticks2)
plot_data(axs[1], x3, y3_vanilla, y3_flexivit, y3_mspe, 16, yticks3)
plot_data(axs[2], x4, y4_vanilla, y4_flexivit, y4_mspe, 24, yticks4)
