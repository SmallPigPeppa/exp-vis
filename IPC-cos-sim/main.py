...
if __name__ == '__main__':
    ...
    fig, axs = plt.subplots(1, 7, figsize=(35, 5), layout='constrained')
    for idx, ax in enumerate(axs.flat):
        pcm = ax.pcolormesh(sim_list[idx], cmap=plt.cm.Blues)
        ax.set_xlabel("Sample Index")
        ax.set_ylabel("Sample Index")
        ax.set_title(f"{key_list[idx]}")

    fig.colorbar(pcm, ax=axs[6], shrink=1.0, location='right')


    plt.show()
    fig.savefig('cos-sim.png', format='png', dpi=300)
