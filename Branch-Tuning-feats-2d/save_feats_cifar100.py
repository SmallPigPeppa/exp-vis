...
def laod_args():
    ...
    parser.add_argument('--ckpt_path', type=str, default='ckpt/byol-bt-task0.ckpt', help='')
    parser.add_argument('--data_dir', type=str, default='/Users/lwz/torch_ds', help='')
    parser.add_argument('--method_name', type=str, default='byol-bt-task0', help='')
...
def load_weight(ckpt_path):
    ...
    state = torch.load(ckpt_path,map_location=torch.device('cpu'))["state_dict"]
...
if __name__ == '__main__':
    ...
    np.save(os.path.join(args.save_dir, f'{args.method_name}_features.npy'), sorted_features[:1000])
    np.save(os.path.join(args.save_dir, f'{args.method_name}_labels.npy'), sorted_labels[:1000])
