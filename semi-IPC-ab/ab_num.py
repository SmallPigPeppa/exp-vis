...
if __name__ == '__main__':
    # 加载配置文件
    config_file = 'ab-num-5tasks.json'
    config_file = 'ab-num-10tasks.json'
    ...
    num = config["plot_settings"]["num"]
    # dataset_name = config["plot_settings"]["dataset_name"]


    # 函数用于绘制每个数据集的图表
    def plot_dataset(ax, dataset_i, num_categories):
        for num_i, settings in num.items():
            data = config["datasets"][dataset_i][num_i]
            ax.plot(num_categories, data, label=num_i, color=settings["color"], linestyle=settings["linestyle"],alpha=settings["alpha"])
